<?php
 include '../component/sidebar.php'
 ?>
include

<style>
    body {
        background-color: #292929;
        background-repeat: no-repeat;
    }
    .card {
        background-color: #292929;
        width: 11rem;
    }
    .card-title {
        color: white;
        text-align: center;
    }
    .btn {
        text-align: center;
        color: white;
    }
    .text {
        color: white;
    }
</style>
<div
    class="container p-6 m-12 h-10"
    style="background-color: #292929; border-top: 20px solid #292929; boxshadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    <table>
        <tr>
            <td>
                <h4 class="text">Genre
                </h4>
            </td>
        </tr>
        </table>
    <hr>
    <div class="scrollmenu">
        <div class="d-flex">
            <div class="card">
                <a href="../genre/supernatural.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Genshin-Impact-4-koma.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">4-Koma</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../weakDemonHunter/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/10/Komik-Weak-Demon-Hunter.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Action</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../tokyorevengers/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-TokyoRevengers.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Adventure</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../tokyorevengers/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-TokyoRevengers.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Cooking</h5>
                    </div>
                </a>
            </div>
            <div class="card">
                <a href="../tokyorevengers/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-TokyoRevengers.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Crime</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Demons</h5>
                    </div>
                </div>
            </a>
        </div>
    </div>
    <br>
    <hr>
    <div class="scrollmenu">
        <div class="d-flex">
            <div class="card">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Drama</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Ecchi</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Fantasy</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Game</h5>
                    </div>
                </a>
            </div>
            <div class="card">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Gender Bender</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../yuanzun/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                    <h5 class="card-title">Harem</h5>
                    </div>
                </div>
            </a>
        </div>
    </div>
    <br>
    <hr>
    <div class="scrollmenu">
        <div class="d-flex">
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Historical</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Horor</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Isekai</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Josei</h5>
                    </div>
                </a>
            </div>
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Life</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Magic</h5>
                    </div>
                </div>
            </a>
        </div>
    </div>
    <br>
    <hr>
    <div class="scrollmenu">
        <div class="d-flex">
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Manhua</h5>
                    </div>
                </div>
            </a>
            <div class="card mx-4">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-Solo-Leveling.jpeg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Manhwa</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Yuan-Zun.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Martial Arts</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../genre/supernatural.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-TokyoRevengers.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Supernatural</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../soloLeveling/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2020/12/Komik-TokyoRevengers.jpg"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Time Travel</h5>
                    </div>
                </div>
            </a>
            <div class="card">
                <a href="../genshinimpact/dashboard.php">
                    <img
                        class="card-imgtop"
                        src="https://i2.wp.com/img.readmanga.cc/uploads/2021/01/Komik-Genshin-Impact-4-koma.png"
                        width="160"
                        height="200">
                    <div class="card-body">
                        <h5 class="card-title">Video Game</h5>
                    </div>
                </div>
            </a>
    </div>
</aside>
<!-- Option 1: Bootstrap Bundle with Popper -->
<script
    src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-
MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM"
    crossorigin="anonymous"></script>
</body>
</html>